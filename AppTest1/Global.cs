...
namespace AppTest1
{
    class Global
    {
        ...
        public void Save()
        {
            ...
            Log.Info("save", "Save complete  " + filePath);
        ...
        public void Load()
        {
            ...
            foreach (var ligne in text)
            {
                ...
                DateTime timeStart = new DateTime(2017, 01, 01, int.Parse(split[3]), int.Parse(split[4]), 0);
                DateTime timeEnd   = new DateTime(2017, 01, 01, int.Parse(split[5]), int.Parse(split[6]), 0);
                if (split.Length > 5)
                {
                    ...
                    ListModel model = new ListModel(bool.Parse(split[1]), split[0], split[2], null, timeStart, timeEnd,null);
                    list.Add(model);
                }
                else
                {
